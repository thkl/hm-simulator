...
module.exports = class Rega {
  constructor (storagePath) {
    ...
    try {
      ...
      this.server.listen(8181, '0.0.0.0', () => {})
    } catch (e) {
      logger.error(e)
    }
  }
  ...
  createEventServer () {
    let self = this
    let eventServer = xmlrpc.createServer({host: '0.0.0.0', port: 2000})
          ...
          // its a channel
            ...
            if (rchan === null) {
              ...
              try {
                ...
                logger.error('error while getParamsetDescription for %s', object.ADDRESS)
              }
            }
      ...
      client.methodCall('init', ['xmlrpc://127.0.0.1:2000', intf.Name()], (error, value) => {
        ...
        // load devices
          ...
          client.methodCall('listDevices', [intf.Name()], async (error, value) => {
            if (!error) {
              try {
                logger.info('loading devices for %s', intf.Name())
